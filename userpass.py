...
try:  import argparse
  ...
  print '[!] "debian based" run > apt-get install python-argparse'; exit()
...
Examples:
  ...
  python %(prog)s -d David Stallman -o out.txt -s -u -a "@ * #" -rc''')
...
if data and outfile:
  ...
  if data:                                                                          # Shuffling Data in heuristic way
    ...
    try:
      ...
      else: print '[+] This operation may take some time ...'
      for i in range(thread):
